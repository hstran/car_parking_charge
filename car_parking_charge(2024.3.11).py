...
class ParkingCharge:
    def __init__(self):
        self.file_path = 'F:\\电信\\智慧交通项目\\2024.2.23\\6_1.xlsx'
        self.free_spots = 83
    ...
    def calculate_fee(self, plate, entry, exit):
        ...
        # 计费函数化，重复调用，填入不同参数
        def charge_fee(hours, rate, cap):
            return min(rate * math.ceil(hours), cap)
        ...
        if parking_time_hours <= 0.5:
            ...
            # 1小时内收费4元
            fee = charge_fee(parking_time_hours, 4, 4)
        elif parking_time_hours <= 12:
            fee = charge_fee(parking_time_hours, 4, 15)
        elif parking_time_hours <= 24:
            fee = charge_fee(parking_time_hours - 12, 4, 25) + 15
        else:
            fee += 25 * (parking_time_hours // 24)
            remaining_hours = parking_time_hours % 24
            if remaining_hours <= 0.5:
                ...
                fee += charge_fee(remaining_hours, 4, 4)
            elif remaining_hours <= 12:
                fee += charge_fee(remaining_hours, 4, 15)
            else:
                fee += charge_fee(remaining_hours, 4, 25)
